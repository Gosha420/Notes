 ...
 event.respondWith((async()=>{
  try{
   ...
   if(event.request.mode==='navigate'){
    const patched=await withPatch(response);
    caches.open(CACHE).then(c=>c.put(event.request,patched.clone()));
    return patched;
   }
   caches.open(CACHE).then(c=>c.put(event.request,response.clone()));return response;
  }catch(_){
   const cached=await caches.match(event.request)||await caches.match('./index.html');
   if(cached&&event.request.mode==='navigate')return withPatch(cached);
   return cached;
  }
 })());
});